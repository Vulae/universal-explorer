use util_vfs::{OsFs, VirtualFileSystem};
...
impl App {
    ...
    fn list_features(&self, ui: &mut egui::Ui) {
        const FEATURES_ENABLED: [(&str, bool); 3] = [
            ("archive", cfg!(feature = "archive")),
            ("source-engine", cfg!(feature = "source_engine")),
            ("renpy", cfg!(feature = "renpy")),
        ];

        ui.vertical_centered(|ui| {
            ui.label("Enabled Features");
            ui.separator();
            for (name, enabled) in FEATURES_ENABLED.iter() {
                ui.horizontal(|ui| {
                    if *enabled {
                        ui.add(
                            egui::Image::new(assets::LUCIDE_CHECK.to_owned())
                                .tint(egui::Color32::LIGHT_GREEN)
                                .shrink_to_fit(),
                        );
                    } else {
                        ui.add(
                            egui::Image::new(assets::LUCIDE_X.to_owned())
        ...
        egui::TopBottomPanel::top("universal-explorer_decorations")
            ...
            .show(ctx, |ui| {
                ...
                egui::MenuBar::new().ui(ui, |ui| {
                    ...
                    ui.menu_image_button(assets::LUCIDE_FLAG.to_owned(), |ui| {
                        self.list_features(ui);
                    });

                    ui.menu_image_button(assets::LUCIDE_PLUS.to_owned(), |ui| {
