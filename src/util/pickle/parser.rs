...
impl Stack {
    ...
    pub fn last(&self) -> Result<&Value> {
        match self.stack.last() {
            Some(StackItem::Value(value)) => Ok(value),
            Some(StackItem::Mark) => Err(anyhow!("Cannot get StackItem::Mark as last item in stack")),
...
impl Parser {
    pub fn new() -> Self {
        Self { protocol: None, stack: Stack::new(), memo: Memo::new() }
    }
    ...
    fn read_operation(&mut self, data: &mut impl Read) -> Result<Opcode> {
        ...
        match opcode {
            ...
            Opcode::STOP => { },
            Opcode::FRAME => { reader.read::<u64>()?; }, // A hint for how many bytes to read in the pickle object.
            Opcode::EMPTY_DICT => self.stack.push(Value::Dict(HashMap::new())),
            Opcode::BINPUT => self.memo.set(reader.read::<u8>()? as usize, self.stack.last()?.clone()),
            Opcode::MARK => self.stack.push_mark(),
            Opcode::BINUNICODE => self.stack.push(Value::String(reader.read_length_string::<u32>()?)),
            ...
            Opcode::SHORT_BINSTRING => self.stack.push(Value::String(reader.read_length_string::<u8>()?)),
            Opcode::TUPLE3 => {
            ...
            Opcode::APPEND => {
                ...
                match self.stack.last_mut()? {
                    ...
                    _ => return Err(anyhow!("Cannot Opcode::APPEND onto value that is not a list")),
                }
            },
            Opcode::BINGET => {
            ...
            Opcode::LONG_BINPUT => self.memo.set(reader.read::<u32>()? as usize, self.stack.last()?.clone()),
            Opcode::SETITEMS => {
                let mut items = self.stack.pop_mark()?;
                let dict = match self.stack.last_mut()? {
                    Value::Dict(dict) => dict,
                    Value::Class(class) => &mut class.data,
                    _ => return Err(anyhow!("Cannot Opcode::SETITEMS onto value that is not dict or class")),
                };
                while items.len() > 0 {
                    let key = match items.pop() {
                        Some(Value::String(string)) => string,
                        Some(_) => return Err(anyhow!("Opcode::SETITEMS key item is expected to be Value::String")),
                        None => return Err(anyhow!("Opcode::SETITEMS failed to get key")),
                    };
                    let value = items.pop().ok_or(anyhow!("Opcode::SETITEMS failed to get value"))?;
                    dict.insert(key, value);
                }
            },
            Opcode::MEMOIZE => self.memo.push(self.stack.last()?.clone()),
            Opcode::SHORT_BINUNICODE => self.stack.push(Value::String(reader.read_length_string::<u8>()?)),
            Opcode::SHORT_BINBYTES => {
                let length = reader.read::<u8>()?;
                self.stack.push(Value::Binary(reader.read_buf(length as usize)?));
            },
