...
impl<F: Read + Seek + Clone> crate::util::virtual_fs::VirtualFsInner<F> for TreeFs<F> {
    fn read(&mut self, path: &str) -> Result<crate::util::virtual_fs::VirtualFsInnerEntry<F>> {
        ...
        Ok(match current {
            TreeNode::File(file) => crate::util::virtual_fs::VirtualFsInnerEntry::File(file.clone()),
            TreeNode::Directory(entries) => crate::util::virtual_fs::VirtualFsInnerEntry::Directory(entries.iter().map(|e| e.0.clone()).collect::<Vec<_>>()),
        })
    }
}
